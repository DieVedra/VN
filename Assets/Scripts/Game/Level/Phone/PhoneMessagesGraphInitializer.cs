    ...
    public void InitMessagesGraphs(IReadOnlyList<PhoneDataProvider> dataProviders, IReadOnlyList<PhoneContactsProvider> contactsToSeriaProviders)
    {
        ...
        for (int i = 0; i < dataProviders.Count; i++)
        {
            ...
            for (int j = 0; j < phoneDataProvider.PhoneDatas.Count; j++)
            {
                ...
                {
                    phoneContactData = phoneDataProvider.PhoneDatas[j].PhoneContactDatas[k];
                    if (phoneContactData.PhoneMessagesGraph != null)
                    {
                        for (int l = 0; l < phoneContactData.PhoneMessagesGraph.nodes.Count; l++)
                        {
                            TryInitPhoneMessagesNode(phoneContactData.PhoneMessagesGraph.nodes[l], seriaIndex);
                        }
                    }
                }
            }
        ...
        {
            for (int j = 0; j < contactsToSeriaProviders[i].PhoneContactDatas.Count; j++)
            {
                phoneContactData = contactsToSeriaProviders[i].PhoneContactDatas[j];
                if (phoneContactData.PhoneMessagesGraph != null)
                {
